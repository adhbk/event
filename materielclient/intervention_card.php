...
/**
 *   	\file       intervention_card.php
 *		\ingroup    materielclient
 *		\brief      Page to create/edit/view intervention
 */
...
dol_include_once('/materielclient/class/intervention.class.php');
dol_include_once('/materielclient/lib/materielclient_intervention.lib.php');
...
$contextpage= GETPOST('contextpage', 'aZ')?GETPOST('contextpage', 'aZ'):'interventioncard';   // To manage different context of search
...
$hookmanager->initHooks(array('interventioncard','globalcard'));     // Note that conf->hooks_modules contains array
...
//$isdraft = (($object->statut == Intervention::STATUS_DRAFT) ? 1 : 0);
...
if (empty($reshook))
{
    ...
    $backurlforlist = dol_buildpath('/materielclient/intervention_list.php', 1);
    if (empty($backtopage)) {
        if (empty($id)) $backtopage = $backurlforlist;
        else $backtopage = dol_buildpath('/materielclient/intervention_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
    }
    $triggermodname = 'MATERIELCLIENT_INTERVENTION_MODIFY';	// Name of trigger action code to execute when we modify record
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $trigger_name='INTERVENTION_SENTBYMAIL';
    $autocopy='MAIN_MAIL_AUTOCOPY_INTERVENTION_TO';
    $trackid='intervention'.$object->id;
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Intervention")));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = interventionPrepareHead($object);
	dol_fiche_head($head, 'card', $langs->trans("Intervention"), -1, $object->picto);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/materielclient/intervention_list.php', 1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('intervention'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/materielclient/intervention_agenda.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'intervention', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
