...
class FormGroupeMateriel extends Form{
	 ...
	 */
    public function selectForFormsGroupe($objectdesc, $htmlname, $preselectedvalue, $showempty = '', $searchkey = '', $placeholder = '', $morecss = '', $moreparams = '', $forcecombo = 0, $idtiers,$isExterieur)
	{
		...
		else
		{
			...
			$out.=$this->selectForFormsListGroupe($objecttmp, $htmlname, $preselectedvalue, $showempty, $searchkey, $placeholder, $morecss, $moreparams, $forcecombo,0,$idtiers,$isExterieur);
		}
	 ...
	 * @see selectForForms()
	 */
    public function selectForFormsListGroupe($objecttmp, $htmlname, $preselectedvalue, $showempty = '', $searchkey = '', $placeholder = '', $morecss = '', $moreparams = '', $forcecombo = 0, $outputmode = 0, $idtiers,$isExterieur)
	{
		...
		if($idtiers)
			if($isExterieur)
				$sql .= ' WHERE t.rowid IN (SELECT m.rowid FROM llx_materielclient_materiel as m JOIN llx_societe as t ON m.fk_soc = t.rowid WHERE t.rowid = '.$idtiers.' AND m.emplacement = 1)';
            else
				$sql .= ' WHERE t.rowid IN (SELECT m.rowid FROM llx_materielclient_materiel as m JOIN llx_societe as t ON m.fk_soc = t.rowid WHERE t.rowid = '.$idtiers.' AND m.emplacement = 2)';
		else
			$sql .= ' WHERE 1=1';
